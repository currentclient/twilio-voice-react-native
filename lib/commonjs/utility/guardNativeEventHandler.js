...
exports.guardNativeEventHandler = guardNativeEventHandler;
 ...
 */
function guardNativeEventHandler(scope, handler) {
  return (...args) => {
    try {
      handler(...args);
    } catch (error) {
      console.error(`[twilio-voice-react-native] Uncaught error handling a "${scope}" ` + 'native event; dropping this event rather than letting it crash ' + 'the app:', error);
    }
  };
}
//# sourceMappingURL=guardNativeEventHandler.js.map