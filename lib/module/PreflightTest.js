...
export class PreflightTest extends EventEmitter {
  ...
  constructor(_uuid) {
    ...
    this._uuid = _uuid;
    common.NativeEventEmitter.addListener(Constants.ScopePreflightTest, common.guardNativeEventHandler(Constants.ScopePreflightTest, this._handleNativeEvent));
...
function parseCallQualityIos(nativeCallQuality) {
  ...
  if (typeof parsedCallQuality !== 'string') {
    throw new InvalidStateError(`Call quality invalid. Expected one of "Excellent", "Great", "Good", ` + `"Fair", "Degraded", found "${nativeCallQuality}".`);
  }
  return parsedCallQuality;
}
